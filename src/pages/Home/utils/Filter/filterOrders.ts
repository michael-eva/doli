...
export function filterOrders(
  ...
  if (!isChecked && locationFilter) {
    ...
    filterPosts = filterPosts.filter((post) => {
      ...
      return (
        lowercaseSuburb && lowercaseSuburb.includes(lowercaseLocationFilter)
      );
    });
  }
  ...
  if (isChecked && decodedLocationFilter) {
    ...
    filterPosts = filterPosts.filter((post) =>
      isCoordinateWithinRadius(
        {
          latitude: post.locationData?.coordinates?.latitude,
          longitude: post.locationData?.coordinates?.longitude,
        },
  ...
  if (searchFilter && searchFilter.trim() !== "") {
    filterPosts = filterPosts.filter((post) => {
      ...
      return columnsToSearch.some((column) => {
        ...
        return (
          typeof value === "string" && containsSearchText(value, searchFilter)
        );
      });
    });
  }
  ...
  return {
    numberOfPosts: postslength,
    filterPosts,
    paginatePageVar: paginatePage(currentPage, pageSize, filterPosts),
    isLoading,
    fetchData,
  };
}
